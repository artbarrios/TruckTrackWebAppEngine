...
{
    class PalletsIndexPrinterFriendlyReport
    {
        public static void Generate(Report report, string fileSaveDirectory, Application app)
        {
            ...
            {
                string message = AppCommon.AppendInnerExceptionMessages("PalletsIndexViewReport.Generate: " + e.Message, e);
        ...
        private static void AddDocumentBody(Document document)
        {
            ...
            List<Pallet> palletsWebData = new List<Pallet>();
            palletsWebData = Web_Data.PalletsWebData.GetPallets();
            ...
            table.Rows[table.Rows.Count].Cells[2].Range.Text = "Value";
            table.Rows[table.Rows.Count].Cells[3].Range.Text = "Load";
            ...
            // add table data rows
            foreach (Pallet pallet in palletsWebData)
            {
                table.Rows.Add();
                // format data row
                ...
                table.Rows[table.Rows.Count].Cells[1].Range.Text = pallet.Name.ToString();
                table.Rows[table.Rows.Count].Cells[2].Range.Text = pallet.Value.ToString();
                table.Rows[table.Rows.Count].Cells[3].Range.Text = pallet.LoadId.ToString();
